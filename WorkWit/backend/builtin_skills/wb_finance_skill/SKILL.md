---
name: "wb_finance_skill"
display_name: "金融分析总入口"
...
source_name: "wb-finance-skill"
create_source: "builtin"
---

> ⚠️ **环境适配**：本技能原文要求调用 agentic_search（WorkBuddy 金融专用检索），当前系统无该工具，金融数据检索统一用 `web_search` / `web_fetch`；通达信 MCP 未接入时同样走 web_search 兜底。红线、时区口径、输出规范保持适用。

> 📎 **参考文件说明**：正文中引用的 references/、rules/、workflows/ 等子文件未随技能导入（系统仅存单 SKILL.md），请直接依据本文方法论执行，不要尝试读取不存在的参考文件。

## 触发顺序硬约束
- 必须**先**加载 wb-finance-skill 读取红线、时区口径、路由与输出规范，任何涉及金融市场数据的请求，**都要**调用 web_search 工具检索数据；**禁止跳过本 skill 直接裸答或者凭记忆回答。**
...
## 检索策略
- 金融数据检索统一经 **web_search 工具** —— 它**具备自主分析与多步规划能力**，会自行判断查哪些维度、分几步查，返回结论。
...
- 工具无法满足时，用 web_search 检索公开信息，明确告知用户数据来源并说明非实时性。

## 数据底线

- **前提显式**：问操作类问题（买/卖/加仓/减仓/换股）时，先列前提（市场环境 + 用户风险偏好 + 资金量/期限），再给"条件 → 操作 → 风险提示"。前提缺失时主动追问而非直接给操作建议
- **检索优先于记忆**：提及具体股票/基金/指数/宏观指标时，先调 web_search；如通达信 MCP 可用，按MCP场景说明调用拉数据，禁止纯凭记忆作答；记忆中的数字只能作为合理性 sanity check，不能作为答案
...
- **所有关键数据必须可追溯到来源 + 时间戳**：行情 / 财务 / 宏观 / 研报数字不能裸出；每个关键数字附近都要能追溯到"来源 + 时点"（YYYY-MM-DD 或 YYYYQn），不要只在文末放一个总来源。同一数据块共享相同来源、周期和口径时，可在表头、表尾或图注统一标注，来源或周期不同时，再分别标注。来源可来自 web_search / 通达信 MCP / 交易所公告 / 公司年报 / 港交所披露易 / 研报 / web_search；web_search 兜底时也要标媒体名 + 日期，若生成 HTML，最好把 web_search 原文链接做成可点击链接。研报和媒体数据要标清"非一手来源 / 需核实原文"，不要把它们和公司公告同等处理
...
1. **识别意图**：先分清这是"取数据"（→ 委派 web_search 工具）还是"给方法论 / 分析 / 输出"（→ 读对应 reference、跑 scripts）；很多请求两者都要（先取数再分析）
2. **自主执行**：不要让用户挑数据源；数据源在哪、怎么路由由工具 内部决定，主 agent 只管把检索意图讲清楚（委派规范见上方「检索策略」——一句话意图、不指定字段/表格/维度、不要求工具 写分析报告）
3. **错误兜底**：工具 返回缺失或报错时，换个问法再调用，或用通达信 MCP（如可用）/ web_search 补
...
- 方法论类 references 只管"分析框架"，**数据获取走 web_search 工具 / 通达信 MCP（如可用）**
